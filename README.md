...
For troubleshooting, RVTools can produce debug logging through its logging configuration. Debug logging is disabled by default and can impose additional performance overhead when enabled, so it should normally be activated only when diagnostic information is required. In large environments, administrators should also avoid enabling expensive display fields unnecessarily, particularly folder and vApp information, because these options can increase collection time.
